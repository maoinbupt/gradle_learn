...
public class VideoEnabledWebChromeClient extends WebChromeClient implements OnPreparedListener, OnCompletionListener,
     ...
     */
    public VideoEnabledWebChromeClient(View activityNonVideoView, ViewGroup activityVideoView, View loadingView,
                                       VideoEnabledWebView webView, TextView titleView) {
        this.activityNonVideoView = activityNonVideoView;
        this.activityVideoView = activityVideoView;
        this.loadingView = loadingView;
     ...
     *
     * @param callback A VideoEnabledWebChromeClient.ToggledFullscreenCallback
     *                 callback
     */
    ...
    public void onShowCustomView(View view, CustomViewCallback callback) {
        if (view instanceof FrameLayout) {
            ...
            activityVideoView.addView(videoViewContainer, new LayoutParams(LayoutParams.MATCH_PARENT,
    ...
    @Override
    public void onShowCustomView(View view, int requestedOrientation, CustomViewCallback callback) // Only
    // available
    // in
    // API
    // level
    ...
    public void onHideCustomView() {
        // This method must be manually (internally) called on video end in the
        // case of VideoView (typically API level <11)
        // This method must be manually (internally) called on video end in the
        // case of HTML5VideoFullScreen (typically API level 11+) because it's
        // not always called automatically
        // This method must be manually (internally) called on back key press
        // (from this class' onBackPressed() method)
        if (isVideoFullscreen) {
            ...
            try {
                ...
                // https://bugly.qq.com/search?app=900006681&pid=1&ptag=1017-10000&ver=all&bi=all&cl=all&ct=all&s=all&dt=all&sy=stack&sc=onHideCustomView
            }
    ...
    // called in the case of VideoView
    ...
    public void onReceivedTitle(WebView view, String h5title) {
        if (titleView != null && !StringUtils.isEmpty(h5title)) {
            String url = view.getUrl();
            // 某些手机在获取不到标题时会返回url，如果是，则不设置标题
            if (StringUtils.isNotBlank(url) && url.contains(h5title)) {
                return;
            }
            titleView.setText(h5title);
        }
